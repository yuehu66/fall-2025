...
function variance_quadrature()
    ...
    println("More grid points means better approximation.")
end
...
function optimize_mlogit(X, Z, y)
    ...
    # call the optimizer
    result = optimize(td, startvals, LBFGS(), Optim.Options(g_tol = 1e-5, iterations=100_000, show_trace=true))
...
function allwrap()
    ...
    println("Same interpretation as PS3; this time it's larger and positive with t-stat of >100")
